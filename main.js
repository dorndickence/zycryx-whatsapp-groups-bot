...
global.prefix = new RegExp(
  "^[" +
    (opts["prefix"] || "xzXZ/i!#$%+£¢€¥^°=¶∆×÷π√✓©®:;?&.\\-HhhHBb.aA").replace(
      /[|\\{}()[\]^$+*?.\-\^]/g,
      "\\$&"
    ) +
    "]"
...
global.db = new Low(
  /https?:\/\//.test(opts["db"] || "")
    ...
    : new JSONFile(`${opts._[0] ? opts._[0] + "_" : ""}${dirP}database.json`)
);
...
const connectionOptions = {
  ...
  auth: state,
  browser: ["ZycryxBot", "Safari", "1.0.0"],
};
...
if (opts["server"])
  (await import(join(__dirname, "server.js"))).default(global.conn, PORT);
...
function actualizarNumero() {
  ...
  const updatedConfigData = configData.replace(
    /(global\.animxscans\s*=\s*\[\s*\[')[0-9]+'(,\s*'Bot principal\s*-\s*CuriosityBot-MD',\s*true\]\s*\])/,
    function (match) {
      const archivoCreds = readFileSync(
...
function cleanupOnConnectionError() {
  ...
  try {
    ...
    console.log(
      `No se pudo eliminar el archivo de copia de seguridad o no existe: ${backupFilePath}`
    );
  }
  process.send("reset");
}
...
function credsStatus() {
  ...
  try {
    ...
    console.log(
      `El archivo de credenciales no existe o está vacío. Generando código QR...`
    );
    connectionOptions;
  ...
  if (!originalFileValid) {
    ...
    if (backupStats.isFile() && backupStats.size > 0) {
      ...
      console.log(
        `Archivo de credenciales restaurado desde la copia de seguridad: ${backupFilePath} -> ${credsFilePath}`
      );
      process.send("reset");
    } else {
      console.log(
        `No se encuentra el archivo de credenciales válido y el archivo de copia de seguridad no es válido o falta: ${credsFilePath}, ${backupFilePath}`
      );
      connectionOptions;
    }
  } else {
...
async function connectionUpdate(update) {
  ...
  if (connection === "open") {
    ...
    if (existsSync(global.authFile)) {
      console.log(
        chalk.green("✓ Archivo de credenciales guardado correctamente")
      );
    } else {
      console.log(
        chalk.yellow("🚩ㅤError al guardar el archivo de credenciales")
      );
    }
    backupCreds();
    actualizarNumero();
...
global.reloadHandler = async function (restatConn) {
  ...
  conn.sSubject =
    "📝 *SE HA MODIFICADO EL TÍTULO DEL GRUPO*\n*NUEVO TITULO:* @subject";
  conn.sIcon = "🥏 *SE HA CAMBIADO LA FOTO DEL GRUPO!!*";
  conn.sRevoke =
    "🥏 *SE HA ACTUALIZADO EL ENLACE DEL GRUPO!!*\n*NUEVO ENLACE:* @revoke";
...
global.reload = async (_ev, filename) => {
  if (pluginFilter(filename)) {
    ...
    if (err)
      conn.logger.error(
        `Error de sintaxis mientras se carga '${filename}'\n${format(err)}`
      );
    else
      try {
        ...
        conn.logger.error(
          `Hay un error que requiere atención en '${filename}\n${format(e)}'`
        );
      } finally {
  ...
  listaDirectorios.forEach((filesInDir) => {
    ...
    const DSBPreKeys = directorio.filter((fileInDir) => {
      ...
      const stats = statSync(
        path.join(join(__dirname, jadibts + filesInDir + "/" + fileInDir))
      );
      const mtime = new Date(stats.mtime);
      const now = new Date();
      const hourAgo = new Date(now - 60 * 60 * 1000);
