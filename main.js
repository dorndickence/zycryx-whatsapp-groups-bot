...
const connectionOptions = {
  ...
  getMessage: async (key) => {
    if (store) {
      ...
      return conn.chats[key.remoteJid] &&
        conn.chats[key.remoteJid].messages[key.id]
        ? conn.chats[key.remoteJid].messages[key.id].message
        : undefined;
    }
...
async function connectionUpdate(update) {
  ...
  if (update.qr != 0 && update.qr != undefined) {
    console.log(
      chalk.yellow(
        "🚩ㅤEscanea este codigo QR, el codigo QR expira en 60 segundos."
      )
    );
  }
  if (connection == "open") {
...
function clockString(ms) {
  ...
  return [d, " Día(s) ️", h, " Hora(s) ", m, " Minuto(s) ", s, " Segundo(s) "]
