...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!userSP) {
    ...
    await conn.reply(
      m.chat,
      `*@${m.sender.split("@")[0]} REGISTRADO EN EL JUEGO* ✅`,
      m,
      { mentions: [m.sender] }
    );
  }
  ...
  async function generarSopaDeLetras() {
    ...
    while (!palabraAgregada) {
      ...
      for (let i = 0; i < PALABRA.length; i++) {
        if (DIRECCION === "horizontal" && columnaInicial + i >= LADO) {
          ...
          DIRECCION === "diagonalDerecha" &&
          (filaInicial + i >= LADO || columnaInicial + i >= LADO)
        ) {
          palabraEntra = false;
          break;
        } else if (
          DIRECCION === "diagonalIzquierda" &&
          (filaInicial + i >= LADO || columnaInicial - i < 0)
        ) {
          palabraEntra = false;
          break;
        }
      ...
      if (palabraEntra) {
        for (let i = 0; i < PALABRA.length; i++) {
          if (DIRECCION === "horizontal") {
            ...
            sopaDeLetras[filaInicial + i][columnaInicial + i] =
              PALABRA.charAt(i);
          } else {
            sopaDeLetras[filaInicial + i][columnaInicial - i] =
              PALABRA.charAt(i);
    ...
    for (let i = 0; i < LADO; i++) {
      ...
      for (let j = 0; j < LADO; j++) {
        if (sopaDeLetras[i][j]) {
          ...
          let letraAleatoria = LETRAS_POSIBLES.charAt(
            Math.floor(Math.random() * LETRAS_POSIBLES.length)
          );
          fila += letraAleatoria + " ";
        }
      }
    ...
    //sopaDeLetrasConBordes += "   *╰" + "┄".repeat(LADO) + '┄┄' + "╯*"
    sopaDeLetrasConBordes = sopaDeLetrasConBordes.replace(
      /[a-zA-Z]/g,
...
*ESCRIBA EL NÚMERO DE FILA Y COLUMNA DEL COMIENZO DE LA PRIMERA LETRA _"${PALABRA.charAt(
        0
      )}"_ DE LA PALABRA _"${PALABRA}"_ TIENE _${intentos}_ INTENTOS!!*
...
➡️ \`\`\`FILA 2\`\`\`    ⬇️ \`\`\`COLUMNA 8\`\`\``.trim()
  ...
  if (userSP != m.sender.split("@")[0]) {
    await conn.reply(
      m.chat,
      `*@${tagUser.split("@")[0]} ESTA JUGANDO SOPA DE LETRAS 🔠 ACTUALEMENTE*`,
      m,
      { mentions: [tagUser] }
    );
    return;
  ...
  if (intentos === 0) {
    ...
    async function resetUserSP() {
      ...
      if (intentos !== 0) {
        await conn.reply(
          m.chat,
          `*@${m.sender.split("@")[0]} TE QUEDA UN MINUTO!!* 😨`,
          m,
          { mentions: [m.sender] }
        );
      }
    ...
    if (`${fila}${columna}` == text) {
      ...
      await m.reply(
        `\`\`\`🎊 HAS GANADO ${diamante} ${rpgshop.emoticon(
          "limit"
      ...
      if (intentos === 1) {
        ...
        intentos -= 1;
        await m.reply(
          `😮‍💨 *INCORRECTO. TE QUEDAN _${intentos}_ INTENTOS!!*${
            intentos === 1
              ? ""
...
handler.command =
  /^(buscarpalabra|sopa|soup|wordsearch|wordfind|spdeletras|spletras|sppalabras|spalabras|spdepalabras)$/i;
