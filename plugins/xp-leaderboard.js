...
let handler = async (m, { conn, args, participants }) => {
  ...
  let len =
    args[0] && args[0].length > 0
      ? Math.min(50, Math.max(parseInt(args[0]), 9))
      : Math.min(9, sortedExp.length);
  let text = `
...
${sortedExp
  ...
  .map(
    ({ jid, exp }, i) =>
      `${i + 1}. ${
        participants.some((p) => areJidsSameUser(jid, p.id))
