  ...
  if (chat.antiTraba && m.text.length > 5000) {
    ...
    if (isAdmin)
      return conn.sendMessage(
        m.chat,
        {
          text: `El administrador @${
            m.sender.split("@")[0]
          } acaba de enviar un texto que contiene muchos caracteres -.-!`,
          mentions: [m.sender],
        },
        { quoted: fakemek }
      );
    await conn.sendButton(
      m.chat,
      `*[❕] Se detecto un mensaje que contiene muchos caracteres [ ! ]*\n`,
      `${isBotAdmin ? "" : "No soy administrador, no puedo hacer nada :/"}`,
      author,
    ...
    if (isBotAdmin && bot.restrict) {
      ...
      return m.reply(
        "[ ! ] Para realizar acciones de eliminación, mi dueño tiene que encender el modo restringido!"
      );
  }
  return !0;
}
