...
async function handler(m, { conn, args, usedPrefix, command }) {
  if (confirmation[m.sender])
    return m.reply("*⚠️ Estas haciendo una transferencia*");
...
✨ *exp* = experiencia`.trim();
  ...
  if (!(who in global.db.data.users))
    return m.reply(`*⚠️ Usuario ${who} no está en la  database*`);
  if (user[type] * 1 < count)
    return m.reply(`*⚠️  ${type}  insuficiente para transferir*`);
  let confirm = `
¿Está seguro de que desea transferir *${count}* ${type} a  *@${(
    who || ""
...
`.trim();
  ...
  confirmation[m.sender] = {
    ...
    count,
    timeout: setTimeout(
      () => (m.reply("Se acabó el tiempo"), delete confirmation[m.sender]),
      60 * 1000
    ),
  };
}
...
handler.before = async (m) => {
  ...
  if (/^Si|si$/i.test(m.text)) {
    ...
    if (previous > user[type] * 1 && _previous < _user[type] * 1)
      ...
      m.reply(
        `Error al transferir *${count}* ${type} to *@${(to || "").replace(
          /@s\.whatsapp\.net/g,
          ""
        )}*`,
        null,
