  ...
  try {
    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (xp > 200) m.reply("chirrido -_-"); // Hehehe
        else m.exp += xp;
        if (
          !isPrems &&
          plugin.limit &&
            ...
            if (e.name)
              for (let [jid] of global.owner.filter(
                ...
                m.reply(
                  ...
                  data.jid
                ); //reporte enviado al privado del propietario
              }
            m.reply(text);
          }
        } finally {
    ...
    if (nk.isGroup == false) {
      if (nk.status == "offer") {
        let callmsg = await this.reply(
          ...
          } las llamadas están prohibidas, seras bloqueado`,
          false,
          { mentions: [nk.from] }
        );
        //let data = global.owner.filter(([id, isCreator]) => id && isCreator)
        //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
        let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Julius Sauco;;;\nFN:Azami\nORG:Julius Sauco\nTITLE:\nitem1.TEL;waid=18134039996:+598 9999\nitem1.X-ABLabel:Julius Sauco\nX-WA-BIZ-DESCRIPTION:Situaciones en las que el bot esté involucrado.\nX-WA-BIZ-NAME:Julius Sauco\nEND:VCARD`;
        await this.sendMessage(
          nk.from,
          { contacts: { displayName: "Julius Sauco", contacts: [{ vcard }] } },
          { quoted: callmsg }
        );
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner:
      "⚠️️ *Este comando solo puede ser utilizado por el Creador del bot*",
    ...
    private:
      "⚠️️ *¡¡¡Este comando solo se puede usar en el chat privado del Bot!!!*",
